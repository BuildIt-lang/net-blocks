...
void checksum_module::init_module(void) {	
	...
	net_packet.add_member("checksum", new generic_integer_member<unsigned short>(0), 2);
...
static builder::dyn_var<unsigned short> compute_checksum(packet_t _p) {
	...
	builder::dyn_var<int> eo = net_packet["total_len"]->get_integer(p) + get_headroom();
...
module::hook_status checksum_module::hook_send(builder::dyn_var<connection_t*> c, packet_t p, 
	...
	builder::dyn_var<unsigned short> checksum = compute_checksum(p);
