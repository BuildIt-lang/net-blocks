...
void identifier_module::init_module(void) {
	...
	if (flow_identifier == flow_identifier_t::src_dst_identifier) {
		...
		net_state.register_member<builder::dyn_var<char[MAX_ACTIVE_CONNECTIONS][HOST_IDENTIFIER_LEN]>>("active_remote_host_ids");
	}	
...
module::hook_status identifier_module::hook_ingress(packet_t p) {
	...
	builder::dyn_var<char*> src_host_id = net_packet["src_host_id"]->get_addr(p);
